...
const CustomSearchDropdown = ({
  ...
  const normalizedOptions = useMemo(() => options
    ...
    .filter((option) => option.value || option.value === 0 || option.value === '0'), [options]);
            ...
            {filteredOptions.map((option) => {
              ...
              return (
                <button
                  key={option.key}
                  type="button"
                  className={`custom-search-dropdown__option ${isSelected ? 'is-selected' : ''} ${option.disabled ? 'is-disabled' : ''}`}
                  onClick={() => handleSelect(option)}
                  disabled={option.disabled}
                >
                  {multiple && (
                    <input type="checkbox" readOnly checked={isSelected} disabled={option.disabled} />
                  )}
